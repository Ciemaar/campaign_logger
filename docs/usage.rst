...
The Generator API handles the creation and execution of random
generators.
...
Authentication for the Generator CLI can be provided via the ``--token``
option or by setting the ``CL_GENERATOR_TOKEN`` environment variable.
...
The main Campaign Logger application exposes a JSON:API compliant REST
API at ``https://logger.campaign-logger.com``. You can use the
``LoggerClient`` to manage Campaigns, Logs, Log Entries, and Campaign
Entries (Pages).

Authentication
==============

You must pass your API token to the client. It will be sent in the
...
.. code:: python
   ...
   response = client.create_campaign(
       title="My Epic Campaign", description="A new adventure begins."
   )
...
.. code:: python
   ...
   response = client.update_log_entry(
       entry_id=entry_id, raw_text="The mysterious stranger gave @Jack a fake map."
   )
...
Authentication for the Logger CLI can be provided via the
``--client-id`` and ``--client-secret`` options or by setting the
``CL_LOGGER_CLIENT_ID`` and ``CL_LOGGER_CLIENT_SECRET`` environment
variables.
