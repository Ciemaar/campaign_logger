...
def test_mcp_server_tool_requires_generator(mocker):
    ...
    if tool:
        ...
        # Ensure we test the inner function to hit the specific branch coverage
        with pytest.raises(Exception, match="Generator client not authenticated"):
            tool.fn()
...
def test_mcp_server_tool_requires_logger(mocker):
    ...
    if tool:
        with pytest.raises(Exception):
            asyncio.run(server.call_tool("list_campaigns", {}))
        # Ensure we test the inner function to hit the specific branch coverage
        with pytest.raises(Exception, match="Logger client not authenticated"):
            tool.fn()
