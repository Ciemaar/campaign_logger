...
def test_list_generators(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/api2/generators", json=[
...
def test_create_generator(client):
    with requests_mock.Mocker() as m:
        m.post(
            ...
            json={"id": "new_gen", "name": "New Generator"}
        )
...
def test_generate_random(client):
    with requests_mock.Mocker() as m:
        m.post(
            f"{BASE_URL}/api2/generators/generate",
            json={"result": "random result"}
        )
        model = FullGeneratorModel(name="Test")
...
def test_execute_operation(client):
    with requests_mock.Mocker() as m:
        m.get(
            f"{BASE_URL}/api2/generators/gen1/generate",
...
def test_get_execute_tokens(client):
    with requests_mock.Mocker() as m:
        m.get(
            f"{BASE_URL}/api2/generators/gen1/execute-tokens",
            json=["token1", "token2"]
        )
...
def test_delete_execute_token(client):
    with requests_mock.Mocker() as m:
        m.delete(
            f"{BASE_URL}/api2/generators/gen1/execute-tokens/token1",
            status_code=200
        )
