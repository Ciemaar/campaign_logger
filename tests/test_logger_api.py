...
def test_get_campaigns(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/campaigns", json={"data": [{"id": "c1"}]})
...
def test_get_campaign(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/campaigns/c1", json={"data": {"id": "c1"}})
...
def test_update_campaign(client):
    with requests_mock.Mocker() as m:
        m.patch(f"{BASE_URL}/campaigns/c1", json={"data": {"id": "c1"}})
...
def test_get_logs(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/logs", json={"data": [{"id": "l1"}]})
...
def test_update_log(client):
    with requests_mock.Mocker() as m:
        m.patch(f"{BASE_URL}/logs/l1", json={"data": {"id": "l1"}})
...
def test_get_log_entries(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/log-entries", json={"data": [{"id": "le1"}]})
...
def test_update_log_entry(client):
    with requests_mock.Mocker() as m:
        m.patch(f"{BASE_URL}/log-entries/le1", json={"data": {"id": "le1"}})
...
def test_get_campaign_entries(client):
    with requests_mock.Mocker() as m:
        m.get(f"{BASE_URL}/campaign-entries", json={"data": [{"id": "ce1"}]})
...
def test_update_campaign_entry(client):
    with requests_mock.Mocker() as m:
        m.patch(f"{BASE_URL}/campaign-entries/ce1", json={"data": {"id": "ce1"}})
